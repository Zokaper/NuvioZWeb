...
| B — mode plumbing + Streamlined | **pure half complete** | `playbackModeModels`, `playbackModeRouter`, `playbackSelectionContext`, `playbackQualityOptions`, `playbackSourceSelector` and `streamTraits` ported; 150 tests across the port, zero failures. Instant withheld by `isSelectable` until Phase D. **Remaining:** the quality sheet UI, the progress overlay + failure chain, `playbackStartupWatchdog`, the storage keys and sync mapping, the settings rows, the first-launch selector, the strings, and the `StreamScreen.mount` wiring. Nothing imports any of it yet. |
